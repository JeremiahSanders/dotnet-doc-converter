...
module DocDisplay =
    ...
    let asConsoleString (assemblyDocs : AssemblyDocs) =
        ...
        let members =
            ...
            |> List.map (fun memberInfo ->
                let summary =
                    if memberInfo.Summary = "" then "" else (sprintf ": %s" memberInfo.Summary)

                let remarks =
                    if memberInfo.Remarks = "" then "" else (sprintf "\n%s\n" memberInfo.Remarks)
    ...
    let private getTypeMembers (typeMember:MemberDoc) (memberDocs:MemberDoc seq) =
        ...
        memberDocs
        |> Seq.filter (fun memberDoc -> (memberDoc.FullName <> typeMember.FullName) && memberDoc.FullName.StartsWith(typeMember.FullName))
    ...
    let membersToMarkdown (memberDocs : MemberDoc list) =
        ...
        |> (fun initialStringBuilder ->
              let memberMap =
                  ...
                  |> Map.ofList
              let typeMembers =
                  memberMap.[MemberType.TypeMember]
              typeMembers
              |> List.fold (fun (typeMemberFoldingStringBuilder:StringBuilder) typeMember ->
                  ...
                  |> (fun sb ->
                      ...
                      |> (fun memberNames ->
                              StringBuilder.appendLines memberNames sb
                          )
                    )
                  |> StringBuilder.appendLine ""
    ...
    let private getTypeMemberSummary (memberNode:MemberDoc) : BlockNode list =
        memberNode.Summary
        |> (BlockNode.ofString >> List.singleton)
    let private getTypeMemberRemarks (memberNode:MemberDoc) : BlockNode list =
        memberNode.Remarks
        |> (blockquote >> BlockNode.ofString >> List.singleton)
    let private getTypeMemberBlockNodes (memberNode:MemberDoc) : BlockNode list =
        let getTypeMemberHeader node =
            [ InlineNode.ofString (code node.ShortName); ]
            |> BlockNode.ofNodes
        getTypeMemberHeader memberNode
        |> List.singleton
        |> (fun blockList ->
              if (memberNode.Summary <> "")
              then (List.append blockList (getTypeMemberSummary memberNode))
              else blockList
          )
        |> (fun blockList ->
              if (memberNode.Remarks <> "")
              then (List.append blockList (getTypeMemberRemarks memberNode))
              else blockList
          )
    let private getTypesMembersBlockNodes (typeMember:MemberDoc) (nonTypes:MemberDoc list) : BlockNode list =
        let getNamespaceBlockNode (namespaceValue:string) : BlockNode =
            [(InlineNode.ofString "Namespace: "); (namespaceValue|>(code>>InlineNode.ofString))]
            |> BlockNode.ofNodes
        let getFullNameBlockNode (fullName:string) : BlockNode =
            [(InlineNode.ofString "Full name: "); (fullName|>(code>>InlineNode.ofString))]
            |> BlockNode.ofNodes
        let getTypeHeader (memberDoc:MemberDoc) : BlockNode list =
            (code memberDoc.ShortName)
            |> (h2>>InlineNode.ofString>>BlockNode.ofNode)
            |> (fun nameBlock ->
                [nameBlock;(getNamespaceBlockNode memberDoc.Namespace); (getFullNameBlockNode memberDoc.FullName)]
              )
            |> (fun blockList ->
                  if (memberDoc.Summary <> "")
                  then (List.append blockList (getTypeMemberSummary memberDoc))
                  else blockList
              )
            |> (fun blockList ->
                  if (memberDoc.Remarks <> "")
                  then (List.append blockList (getTypeMemberRemarks memberDoc))
                  else blockList
              )
        nonTypes
    ...
    let assemblyDocsToMarkdownDocumentNode (assemblyDocs : AssemblyDocs) : DocumentNode=
        ...
        |>List.singleton
        |> (fun assemblyHeader ->
          List.append assemblyHeader (getTypeBlockNodes assemblyDocs)
          )
    ...
    let markdownDocumentNodeToString (markdown:Markdown.DocumentNode) =
        markdown.Blocks
        |> List.fold (fun sb block ->
            StringBuilder.appendLine (markdownBlockNodeToString block) sb
            ) (StringBuilder())
        |> StringBuilder.toString
